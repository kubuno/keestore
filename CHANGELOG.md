...
- **Events no longer depend on a PostgreSQL-specific notification.** They are
  announced over the core's internal interface instead, the path the other
  modules already use. Nothing changes for anyone using the module, and an
  event is no longer lost when a subscriber happens to be restarting.
- **The vault's identifier is now generated by the module** rather than by the
  database. Existing vaults keep theirs; this is what lets the module read a
  freshly saved vault back on an engine that cannot return it in one go.
- **The write path moved out of the request handler into the vault service**,
  where it is covered by tests that run against a real server of each of the
  three engines.
