...
<p align="center">
  <img src=".github/logo.png" alt="Kubuno Keestore logo" width="128" height="128">
</p>
...
**Kubuno Keestore — gestionnaire de mots de passe KeePass 4**

A module for [Kubuno](https://github.com/kubuno/core), the self-hosted, libre (AGPLv3) cloud platform.

## Architecture

A standalone Rust process that registers with the [core](https://github.com/kubuno/core) at startup; the core proxies its routes (`/api/v1/keestore/*`) and serves its runtime-loaded React frontend bundle.

- **Backend** — `src/`: Axum + SQLx (PostgreSQL, schema `keestore`); migrations in `migrations/`.
...
bash build_deb.sh                          # → dist/kubuno-keestore_*.deb
```
...
```bash
bash build_rpm.sh        # Fedora / RHEL / openSUSE  → dist/kubuno-keestore-*.rpm
bash build_windows.sh    # Windows installer (NSIS)  → dist/kubuno-keestore-setup-*-x64.exe
bash build_macos.sh      # macOS (run on a Mac)      → dist/kubuno-keestore-*.pkg
```
